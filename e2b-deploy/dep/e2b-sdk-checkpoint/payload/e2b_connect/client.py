...
class ConnectException(Exception):
    def __init__(
        ...
        # `reason` rides alongside the code in the error body. A code says how
        # the RPC ended, not what the caller has to do next - `internal` alone
        # can mean three different next moves - and the reason names the move.
        # `None` against a server that does not send it, which is every server
        # but the checkpoint API.
        self.reason = reason

        # Seconds from a `Retry-After` header, for the statuses that carry one.
...
def retry_after_seconds(http_resp: Response) -> Optional[float]:
    """Seconds from a ``Retry-After`` header, when the response carries one.

    Only the delta-seconds form is read. The other form the header allows is
    an HTTP date, which is answered with ``None`` rather than a guess: the
    servers that send this one send a number of seconds.
    """
...
def error_for_response(http_resp: Response):
    retry_after = retry_after_seconds(http_resp)

    try:
        error = json.loads(http_resp.content)
        return make_error(error, retry_after=retry_after)
    except (json.decoder.JSONDecodeError, KeyError):
        error = {"code": http_resp.status, "message": http_resp.content.decode("utf-8")}
        return make_error(error, retry_after=retry_after)


def make_error(error, retry_after: Optional[float] = None):
    status = None
    try:
...
def _resolve_retries(args, default: int) -> int:
    """How many times to retry this call.

    The decorated functions below are all methods, so `args[0]` is the
    `Client`. Its `_connection_retries` wins over the number baked into the
    decorator, which lets a client be built with `retries=0` for RPCs that
    must not be replayed (creating or restoring a checkpoint is not
    idempotent: a replay makes a second checkpoint, or rolls the guest back a
    second time). The decorator's number stays the default, so clients that do
    not set anything - every envd call - keep the old behaviour.
    """
    if args:
        configured = getattr(args[0], "_connection_retries", None)
...
def _sync_retry(func, exc, retries):
    def retry(*args, **kwargs):
        for _ in range(_resolve_retries(args, retries)):
            try:
...
class Client:
    def __init__(
        ...
        *,
        ...
        # Number of retries on RemoteProtocolError (a peer that closed the
        # connection mid-response). Retrying is only safe for idempotent RPCs;
        # pass retries=0 for anything that changes state on the server.
