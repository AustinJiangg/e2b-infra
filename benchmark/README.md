...
| 创建 firecracker 进程 | 等待firecracker启动 | `configured fc cost` | 启动 FC 进程并等待其 API socket（受 `E2B_FC_LAUNCH_MODE` 4 档影响，档位机制详解见 `FC启动优化-netns-exec.md` / `FC启动优化-launch.md` / `FC启动优化-launch-c.md`。曾细分 `fc spawn` / `fc socket wait` 两个子段，因不同档位下统计口径不可比已移除） |
| 创建 firecracker 进程 | 等待uffd sock | `get uffd sock path cost` | |
| firecracker 恢复虚拟机 | 加载快照 | `load snapshot cost` | 父=下面三段之和 |
| firecracker 恢复虚拟机 | └取snapfile路径 | `get snapfile path cost` | 在 `client.go` `loadSnapshot` 埋点 |
| firecracker 恢复虚拟机 | └FC API LoadSnapshot | `fc api load snapshot cost` | FC 恢复内存映射/设备的真实时间，在 `client.go` 埋点 |
| firecracker 恢复虚拟机 | └等uffd就绪 | `wait uffd ready cost` | 等 UFFD 缺页服务就绪，在 `client.go` 埋点 |
| firecracker 恢复虚拟机 | 调用恢复 | `post resume cost` | resumeVM API |
| firecracker 恢复虚拟机 | 设置mmds | `set mmds cost` | |
| firecracker 恢复虚拟机 | 恢复虚拟机 | `resume VM cost` | 上面各项所在函数的总耗时 |
...
| 模板拉取(后台) | snapfile/metafile/memfile/rootfs 就绪、总耗时 | `template snapfile cost` `template metafile cost` `template metafile v1 cost` `template memfile storage cost` `template rootfs storage cost` `template fetch total cost` | `storage_template.go` `Fetch` 埋点：模板文件下载/建缓存流水线（缓存命中时接近 0；未命中时是 resume 等待模板的真实来源） |
| NBD 设备(后台) | backend size/取设备/socket pairs/Connect/等就绪、总耗时 | `nbd backend size cost` `nbd get device cost` `nbd socket pairs cost` `nbd connect cost` `nbd status wait cost` `nbd open total cost` | `nbd/path_direct.go` `Open` 埋点：rootfs overlay 背后的 NBD 设备打开各阶段 |
