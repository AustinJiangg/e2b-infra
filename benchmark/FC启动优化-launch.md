...
> `FC启动优化-launch-c.md`（档位 4，本档的单线程 C 重写，消掉 unshare(1) 包装与 Go runtime 自启动）。
...
| 每沙箱成本 | 档1 disabled | 档2 netns-exec | 档3 launch | 档4 launch-c |
|---|---|---|---|---|
| fork/exec 次数 | ~8 | ~7 | 1 次 clone + 3 次 execve（unshare→fc-launch→firecracker） | 1 次 clone + **2 次** execve（fc-launch-c→firecracker） |
| 全挂载树递归遍历（持全局锁） | 2 次（rprivate + ip 的 rslave） | 1 次（rprivate） | **0 次**（O(路径深度) 定点标记） | **0 次**（同左） |
| mount ns 创建位置 | 子进程 unshare(1) | 子进程 unshare(1) | 子进程 unshare(1)——v1 曾放在父进程 clone(2)（Cloneflags），100 并发下 spawn 车队，已废弃，见 §3.2 | 子进程 **helper 自身** unshare(2)（单线程 C 合法） |
| /sys 卸载重挂 | 有 | 无 | 无 | 无 |
| shell 解析 | bash | bash | 无 | 无 |
| helper runtime 自启动 | —（bash/coreutils） | Go ~1-2ms | Go ~1-2ms + unshare(1) 一层 | **C，<0.5ms** |
| 动态链接加载 | unshare/bash/mount/ln/mkdir/ip | 少了 ip | 仅 unshare（只链 libc） | 仅 helper 自身（只链 libc） |
| 等 API socket | 10ms 轮询 | 10ms 轮询 | **inotify**（µs 级唤醒） | **inotify**（同左） |
| 隐藏的 rec-private remount | 不适用 | 不适用 | 无（`--propagation unchanged`，见 §3.2） | 无（helper 自己控制，不依赖 util-linux 选项） |

不可避免地保留的内核成本：创建 mount ns 时的挂载树复制（`copy_tree`，∝ 主机挂载点数、
持 `namespace_sem`）——各档都一样、都在子进程里发生，属于"每 FC 一个 mount ns"这个设计
...
- helper 仍是 Go 二进制（runtime 自启动 ~1-2ms），且 v2 多一层 unshare(1) 包装
  （一次小 execve）。**→ 已由档位 4（`launch-c`）解决**：单线程 C 重写的 helper 在
  execve 后自己 `unshare(CLONE_NEWNS)`（Go 因 runtime 多线程共享 fs_struct 做不到，这正是
  v1 被迫用 Cloneflags、v2 借用 unshare(1) 的根因），无需外部包装、无 runtime 自启动，
  详见 `FC启动优化-launch-c.md`。
