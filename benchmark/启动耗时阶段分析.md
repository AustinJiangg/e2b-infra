...
| 创建 firecracker 进程 | 等待 firecracker 启动 | `configured fc cost` | **B** `Resume` 内 errgroup 第 1 个 goroutine（`p.configure(...)`，**启动 FC 进程 + 等 API socket**；受 `E2B_FC_LAUNCH_MODE` 4 档影响） | ~402 | **241.614** | 329.14 | 🟰 并行 **(瓶颈)** |
