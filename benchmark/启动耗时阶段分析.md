...
> ³「启动 envd」三行的埋点是后补的（本表其余数值来自补点前的那次 100 并发压测，故 envd 三行当时为空）；
> 　重新压测即可填上。从快照恢复时 envd 已在运行，`start envd`（orchestrator→envd `/init` 往返）通常只有几 ms，
> 　且包含在 `total` 内（见第 7 节的修正说明）。
