...
- **B. `fc/process.go`** → `func (p *Process) Resume(...)` 与 `configure(...)`
  ...
  `configure` 整体计入 `configured fc`＝`cmd.Start()` + `socket.Wait()`）
...
| 创建 firecracker 进程 | 等待 firecracker 启动 | `configured fc cost` | **B** `Resume` 内 errgroup 第 1 个 goroutine（`p.configure(...)`，**启动 FC 进程 + 等 API socket**；受 `E2B_FC_NETNS_EXEC_HELPER` 开关影响） | ~402 | **241.614** | 329.14 | 🟰 并行 **(瓶颈)** |
...
> ³ 标「待测」的行（准入排队、启动 envd 三行）埋点是后补的，
...
1. **orchestrator 侧补桩（最关键，已实现）**：`server/sandboxes.go` 的 `Create` 在 `startingSandboxes.Acquire(...)` 返回后立即打印 `[ResumeSandbox] acquire wait cost`（成功或 300s 超时失败都会记录；失败的没有 `total`，parse_report 不会计入有效沙箱）（traceID 与 `ResumeSandbox` 同一条 trace），parse_report 归到「准入排队(等starting槽位)」这一行 ⇒ 把「等槽位排队」从「真 resume」里拆出来。高并发下这一项可能很大，拆出来才知道瓶颈是排队还是 resume 本身。
   - **踩过的坑（样本为什么曾经=0）**：`Create` 里获取信号量分两条路径——`req.Snapshot==true`（用户暂停后恢复）走 `waitForAcquire()`，普通 `Sandbox.create()`（`Snapshot==false`）走 `else` 分支的内联 `Acquire`。此前只有 `waitForAcquire()` 打了 `acquire wait` 桩，`else` 分支没打，于是 100 并发压测（走的正是 `else` 分支）里排队时间恒为 0（`样本 0`）。现两条路径都已埋点，非快照创建的排队也能读到数。
