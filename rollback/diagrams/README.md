# checkpoint / restore 配图

950 ext4 路线的汇报素材。完整技术文档见 [`../docs/`](../docs/)。
...
| `01/02-*.mmd`、`03-sequence.mmd` | 上述图的 mermaid 源，供需要改内容时用。`01-architecture.mmd` 比文档里内嵌的那版多了优化点标注气泡 —— 正文另有汇总表，不必重复 |

## 怎么选

- **要读懂这套东西** → 看 [`../docs/`](../docs/)，从 [`00-design-overview.md`](../docs/00-design-overview.md) 开始。
...
- **图里不含任何实测数字**，是有意为之：950 上还没有数据，920B 的数字口径不同，
  混进架构图容易被当成 950 的性能承诺。数字集中在 [`../docs/17-observability-and-verification.md`](../docs/17-observability-and-verification.md)，并标注了口径与缺口。
- **`03-vs-native.svg` 里「脏页判据的三方差异」那一块不要删。** 对比基准是我们所基于的
  **ARM 适配版**，而 x86 原生的增量本来就是精确的 —— 少了这块说明，图会被读成
  「我们比 e2b 原生强」，那是不对的。同一段说明在 [`../docs/07-dirty-page-tracking.md`](../docs/07-dirty-page-tracking.md) §5。
...
slidev 工程在相邻目录 `../slides/`，已随本仓库管理，并把三张图和优化点表
串成一套稿子：

```bash
cd ~/projects/e2b-repo/e2b-infra/rollback/slides && npm run dev
```
