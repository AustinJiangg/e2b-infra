...
>
> **状态**：2026-08-29 已在 950 的 HDBSS 硬件标脏路径上通过正确性验收（[§8.1](#81-950--hdbss-硬件标脏2026-08-29)）；
> 性能分档基准尚未在 950 上跑过。
>
> **客户常问的问题**（如「沙箱销毁后快照为什么恢复不了」）收在 [§9](#9-常见疑问)。
>
> **配图**：`diagrams/` 目录的 `01-architecture.svg`、`02-diff-tree.svg`、`03-vs-native.svg`，
...
单机部署下 `<store-root>` 实际是 `/orchestrator/build/checkpoints`：
...
**不需要配置任何环境变量**。2026-08-29 在 950 上实跑，服务端自报的脏页后端即为
`HDBSS（硬件标脏）`，正确性验收 59 项全过（见 [§8.1](#81-950--hdbss-硬件标脏2026-08-29)）。
...
完整对比见 `diagrams/03-vs-native.svg`。要点：
...
### 8.1 950 · HDBSS 硬件标脏（2026-08-29）

在 950 上跑通 `benchmark/checkpoint_verify.py`，**59 项校验全部通过**。这是本方案第一次在
**硬件标脏**路径上得到验证 —— 服务端自报的脏页后端为 `HDBSS（硬件标脏）`，
Firecracker 1.12.1，产物落在根盘 ext4 的 `/orchestrator/build/checkpoints`。
...
| `kill -9` 掉快照前就在跑的心跳进程后回滚到 A | 进程连同 **PID 与启动时刻**一起复活 |
| 树根是全量、后续是增量 | `mem_mode` 分别为 `full` / `incremental` / `incremental` |

最后一项最有说服力：**能复活一个已经被杀掉的进程**，说明恢复回来的是完整内存现场，
而不是重放了什么操作。心跳进程三代同 PID、同启动时刻，也证明打快照没有重启虚机。

客户端墙钟（含网络往返与服务端全部工作；模板 `base`，每代脏内存 128 MB + 根文件系统写入 32 MB）：
...
- 稳定性：200 次回滚 0 失败；
- 分档基准（创建 p50，2 GB 沙箱）：全量 **1.549 s**；增量 0 MB **0.022 s**、64 MB **0.062 s**、256 MB **0.183 s**；
- 恢复：常见档位在几十毫秒量级。

### 8.3 两组数字不能直接相减

950 与 920B 这两组的**模板大小、存储介质、脏页后端三项全都不同**
（真盘 vs loop 卷，硬件标脏 vs 软件写保护），因此不能把差值归因于 HDBSS。
950 那一轮跑的是 `checkpoint_verify.py`，它的定位是**正确性验收**，
耗时只是顺带打印，不构成性能基准。

### 8.4 尚未覆盖

1. **950 上的分档基准** —— `benchmark/checkpoint_bench.py` 还没在 950 上跑过，
   内存 / 文件系统 / 混合三段、各脏页档位、服务端分段耗时都还没有数据；
2. **HDBSS 与软件写保护的对照** —— 收益尚未量化；
3. **`FC_HDBSS_ORDER` 取 1 / 2 / 4 的对比** —— 写密集负载下缓冲区溢出会吃掉收益，值得实测；
4. 完整 `rpmbuild` 流程尚未跑通。

---

## 9. 常见疑问

### 9.1 沙箱销毁后 checkpoint 也没了，那它到底缺什么才能脱离沙箱恢复？

**先确认行为**：沙箱从 orchestrator 的沙箱表移除时触发 `OnRemove` → `RemoveSandbox`，
删掉三张内存表，然后 `os.RemoveAll(<store-root>/<sandbox-id>)` —— 整个目录连同 `layers/` 一并删除。

但因果顺序反过来说更准确：**不是「因为恢复不了所以删」，而是设计上就绑定了沙箱生命周期**；
即使把目录完整拷走保存，也恢复不了。缺的东西分四层。

#### 一、缺活体本身

只有 `RollbackInPlace` 一条恢复路径 —— 往**活着的** Firecracker 对象上写。
kill-and-rebuild 那条 `RestoreFromFiles` 在 D1 决策时连同骨架一起删除了。

Firecracker 侧的 `rollback` 端点第一步就拒绝非活体：

- 虚机必须处于 `Paused` 状态，否则返回 `NotPaused`；
- `validate_topology` 要求快照描述的拓扑与**运行中的虚机**一致（同 vCPU 数、同内存大小、同设备集）——
  它是把状态写到既有的 KVM fd、既有的 GIC 设备、既有的 virtio 对象上，这些对象不存在就无从写起。

还有一处更本质：回滚集 = 树路径并集 **∪ 活跃脏页**。活跃脏页描述的是「当前虚机相对上次快照改了什么」，
这是**当前状态的属性，不是快照的属性**。没有当前状态，这一项无从谈起 —— 当然离线恢复也不需要它，
直接全量加载即可。它不是障碍，是「原地」这个形态的必然产物。

#### 二、磁盘产物不自足

内存那一半**已经自足**：`CHECKPOINT_FULL_ROOT` 默认开，树根是全量捕获，
内容解析必然终止在树内，不回落到模板（见 [§2.6](#26-树根为什么要全量)）。

盘那一半不是。`assembleView` 永远从模板 rootfs 起步再往上叠封存层；`RootfsView` 里的合并 header
是「模板 rootfs 的映射表 + 各封存层的恒等映射」，底座那些映射项指向模板的 build。
封存层只含**被写过的块**，没被写过的块要回模板取。所以脱离模板，磁盘读不出完整内容。

真要可导出，得把层栈压平 —— 把模板底座物化进产物。这件事记录过（「checkpoint 可导出」），
结论是做成独立的 export 接口而不改 create 的默认路径：就地回滚场景下沙箱活着，模板必然在位。

#### 三、账本不加载（信息在，代码不在）

`NewStore` 只做 `MkdirAll`，三张表 `bySandbox` / `bases` / `rootfs` 全是空的，**不从磁盘读**。

磁盘上的信息其实是够的：`manifest.json` 存的是整个 `Entry` 的 JSON（含 `ParentID`、`MemMode`、
`Rootfs` 视图与各文件路径），每个封存层的 `.meta` 存了它持有哪些块偏移，理论上能重建整棵树 ——
**只是没有代码去读**。

这是刻意取舍：磁盘索引是为了事后可查，不是为了跨重启存活；orchestrator 重启本来就会带走
它上面的所有沙箱。

#### 四、缺运行时环境

Firecracker 进程、KVM fd、tap、网络槽位、NBD 设备、UFFD —— 都是活体资源。
原地回滚的全部价值就在于**一个都不重建**。
...
| 磁盘完整镜像 | 层栈压平，把模板底座物化进产物 | 中，且产物体积回到全量 |
| 账本 | `NewStore` 增加一条从 `manifest.json` 扫描重建的路径 | 小 |
| 恢复路径 | 一条 load-from-files：新起 Firecracker 进程 + 加载 snapfile + 挂内存与磁盘 | 中 |

四条补齐之后，得到的东西**基本就是 e2b 原生 snapshot** —— 全量产物、新建进程、跨节点可用，
而那正是原生路径已经做好的事。

所以这不是缺陷，是分工：本方案用「绑定沙箱生命周期」换掉了产物自足性与账本持久化，
换来零拷贝封存、原地回滚、成本只随回退跨度走。
要跨节点、要长期持久化、要崩溃恢复，走原生 snapshot（见 [§0](#0-定位这套东西解决什么)）。
