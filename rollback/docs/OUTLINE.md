...
1. 本方案**没有改动**原生路径，两者并存，解决不同问题。2026-09-11 起原生路径的内存增量另有一处**独立修复**
   （第 21–23 篇），它与本方案共用 `save-dirty-bitmap` 端点、不共用任何产物；「本方案」一词**不包含**它，
   凡引用它的地方要写明「原生路径修复后」；
...
> **2026-08-31：前四部分与第六部分共 23 篇完成**（约 74000 字）。
> **2026-09-10：第五部分 6 篇（24–29）完成，全书 32 篇。** 下表状态已更新。
> **2026-09-21：第 28 篇换成当前代码（`deltabox-dev@4af2872c6`）的分档基准，旧栈数据整体移入新增的
> 第 28A 篇「历史实测数据」（篇号用 `28a`，不顺延后面的篇号）。全书 33 篇。**
...
| 21 | `21-native-increment-diagnosis.md` | 原生 snapshot 的增量为什么不精确 | ● |
| 22 | `22-native-increment-fix.md` | 原生 snapshot 的精确增量：4 KiB 存、2 MiB 拼 | ● |
| 23 | `23-native-increment-cost-and-verification.md` | 原生精确增量的代价、验证与边界 | ● |
...
- **已知的部署陷阱**（§7）：其中 §7.5「orchestrator 每次重启漏掉一整池网络槽位」写清
  基线 `2026.09` 的成因（开机不回收残留 netns + 关闭路径两个缺口）与上游的修复
  （`2026.17`/`2026.24`/`2026.27`），处置是随基线升级解决、本项目不单独打补丁；
  清理步骤不重复，指向 `deploy-docs/06 §8.1`
...
**21 · 原生 snapshot 的增量为什么不精确**（2026-09-14 补）
对原生路径的一处独立修复（infra-arm `jll` `c9a92a5ab`）的诊断篇，不属于本方案；它回答
「为什么修复前不精确」与「为什么本方案的做法学不过来」这两个读者最常问的问题。
- 现象：`pb2.py` 三列并排，判据列 = 导出列、只读也涨、写跟踪列是对的
- 三层拆解（判 / 存 / 填）与锁住粒度的那根链条：guest 页 → uffd 硬检查 → header 逐代继承
- 修复前不精确 = 判据塌缩 × 每代新进程 × 换入即脏；只换判据到不了底（2 MiB 归并地板，`pb4.py`）
- 本方案天生精确的原因（进程内稀疏文件、不经 uffd）与原生不能照抄的原因（跨节点懒加载要映射链）

**22 · 原生 snapshot 的精确增量：4 KiB 存、2 MiB 拼**
- 路线 A（关大页）vs 路线 B（4 KiB 存、2 MiB 拼）；拼接能力本就存在于 `File.ReadAt`
- 三层改法：换 `save-dirty-bitmap`；header 块大小降级而父代映射不重写；缺页三级取源（零拷贝 / 单映射零拷贝 / 池拼）
- 追踪没开时的退路：回退到原判据，产物形态不变

**23 · 原生精确增量的代价、验证与边界**
- 代价与边界：恢复热路径一度 +100~200 ms 的教训；header 膨胀与零页量过不做；平台策略与本方案一致；兼容矩阵
- 验证四类证据；影响面只在原生路径内存侧，本方案零改动。数据只引第 28 篇（表 3-J、3-K）

---

### 第五部分　测试与验证：怎么证明它对、它快

方法与数据分开：24–27 讲方法，一次写定；28 只放数据与判定，随 950 实测逐步填满；29 是客户面的操作篇。
全书的实测数字**只在 28 一处维护**，其他篇引用不复制。

| # | 文件 | 标题 | 状态 |
|---|---|---|---|
| 24 | `24-test-overview.md` | 测试体系总览 | ● |
| 25 | `25-functional-tests.md` | 功能正确性测试 | ● |
| 26 | `26-performance-methodology.md` | 性能测试：指标与方法 | ● |
| 27 | `27-cross-implementation.md` | 性能测试：三套方案的横向对照 | ● |
| 28 | `28-results-and-compliance.md` | 实测结果与达标判定 | ● |
| 28A | `28a-historical-results.md` | 历史实测数据（2026-08 / 09，旧栈） | ● |
| 29 | `29-acceptance-runbook.md` | 上机验收操作 | ● |

**本部分的四条口径（已定，不再讨论）**

1. **客户指标**：checkpoint ≤ 200 ms、restore ≤ 100 ms。**只约束增量 checkpoint 与原地 restore**；
   树根全量是每个沙箱一次性的成本，单列说明，不参与判定。
2. **判定用客户端墙钟 p50 为主、p99 并列**（SDK 调用返回，含网络往返与服务端全部工作，最保守、客户最容易复测）；
   冻结窗口作为解释项列在旁边，不作判定。
3. **增量的代价随脏页量走，不随沙箱内存大小走**。「不同内存快照大小」在本方案里翻译成「脏页量档位」，判定表按档位给。
4. **gsd/CRIU 那套（phz 的进程级实现）**：27 篇只写方法、口径与定性结论，实测数字只在 28 的一张附表里带条件给出。
...
这件事 24 §1.4 当例子讲，25 §2.4 说清判据依赖哪版 SDK，28 §2 结果表逐轮标注 SDK 与判据是否有效。
...
- §2 三层：单元测试（只给位置与规模，引 30，不展开）/ 交付态验收 / 开发态工具箱。
  为什么交付态零依赖、探针故意重复；为什么两套不混用
- §3 **测试矩阵**（全书唯一一张）：性质 → 脚本 → 判据 → 跑在哪 → 现状（950 / 920B / 待测）。约 17 行：
  活体、内存回滚、文件回滚、根目录+删除+权限位、进程复活、树语义、pause 兼容、原生操作兼容矩阵、
  HDBSS 三级证据、稳定性、O(脏页)、链深、分档分布、冻结窗口、持续连打、时机成本、三套对照
- §4 测量纪律：三个时钟 ↔ 三种数字（客户端墙钟 / 服务端 `timings.json` / 冻结窗口）；
  每个数字的**条件标签**（机型、fs、脏页后端、模板大小、二进制 sha、真盘/loop）；
  先自证会失败；空着比错的数强；不同条件的数不相减
- §5 环境与产物：SDK 覆盖层、`.env`、宿主机 vs 非宿主机（哪些列会空）；`reports/` 目录与 `00-context.md` 约定、`tee` 约定

**25 · 功能正确性测试**
- 预备：08、14、16；代码：`checkpoint_verify.py`、`correctness.py`、`pause_verify.py`、`compat_matrix.py`、`hdbss_evidence.py`、`loop.py`
- §1 什么叫「回到那一刻」：三重证据（tmpfs 标记 = 内存、根 fs 标记 = 磁盘、上百 MB blob md5 = 一大片不是一页）；
  最硬的证据——进程带着 PID 和启动时刻复活
- §2 `checkpoint_verify.py`：四类断言；三代现场设计（A/B/C 换掉全部标记；victim 建 / 删 / 保持删；权限位每代不同）；
  先自证有区分度（9 项）；活体判据为什么不能用时间（引 12 §5）；`mem_mode` 断言与它依赖的 SDK 版本；
  **59 项怎么构成**（场景 × 项数表）；怎么读输出
- §3 `correctness.py`：树语义——线性、前滚、分叉跨 LCA、删除语义（隐藏条目账本）、失败语义（拒绝而非降级）；与 verify 的分工（直链 vs 树）
- §4 `pause_verify.py`：那个 bug；为什么 Go 单测全过、所有老验收脚本全过都抓不到它；**O_DIRECT 为什么是关键**
- §5 `compat_matrix.py`：6 个操作组合；OK / REFUSED / BROKEN 的定义（REFUSED 是边界不是缺陷）；
  两个 REFUSED 是什么（checkpoint 不跨 pause/resume；kill 后 connect 本来就不行）。引 16
- §6 `hdbss_evidence.py`：能力 ≠ 数据面；L1/L2/L3；冷/热比的物理含义；920B 4.5–5.4 是负样本。引 7、19
- §7 `loop.py`：为什么 a/b 交替；failures=0；任何内容错误立即停——静默错误比崩溃危险
- §8 偶发问题怎么复现：一次跨分支挂死的追查路径（187 次纯跨支 0 异常 → 40 轮 correctness 0 挂 → RX 描述符线索 → 回滚瞬间灌 RX），
  作为方法讲，一节；脚本在工作区 `tmp/`，不交付

**26 · 性能测试：指标与方法**
- 预备：08、10、11、17 §1；代码：`checkpoint_bench.py`、`bench-ckpt.py`、`timing.py`、`probe-ramp.py`、`freeze_probe.py`
- §1 **指标口径**：客户要求 → 本书的可测定义（按上面四条口径），一张「口径定义表」，28 篇的判定全按它
- §2 负载构造：三种失真（分配成本、写放大——`eval` 造负载实测 5×、放错层）；预热 + 原地覆写；tmpfs = guest RAM；
  文件半用缓冲写 + sync，页缓存双计是本性不是误差；3:1 的理由
- §3 `checkpoint_bench.py`：档位；两遍拍——时机成本 = ① − ②；直接量产物（`st_blocks`、稀疏 `mem_diff`、新封层）；
  三段一条链；unwind restore 表（代价归上一行）；只测一次的定位
- §4 `bench-ckpt.py`：分布（reps / warmup）；全量基线为什么要 N 个新沙箱；每档新沙箱使链深配平；
  **冻结窗口**——`freeze_probe.py` 2.5 kHz 采样器、底噪、可区分 ✓/⚠；restore 浅集 / 深集；60 s 连打与前后 1/3 对比；漂移自查
- §5 `timing.py`：O(脏页) 与 df 增量；链深 5/20/50——深集 / 浅集把「回滚集大小」和「链深」解耦。引 8 §7.1
- §6 `probe-ramp.py`：判别式设计（按代数 / 按累计写入量两种归一化，看哪种收拢）；判定：跟代数走；根因 `manifest.json` 线性增长
- §7 服务端分段：`timings.json` 字段 → 阶段；create 大头在 FC 写内存、restore 大头在 materialize。引 17 §1
- §8 读报告：`bench.json` / `报告.md` 各表；三种体积口径（逻辑 / du / df）；`mem_mode` 列必须全 incremental

**27 · 性能测试：三套方案的横向对照**
- 预备：01、04、20、26；代码：`checkpoint_bench_v2.py`、`native_snapshot_bench.py`
- §1 三套方案摆在一张图上：层次 / 快照里装什么 / 产物落哪 / 脚本。
  「快照只能带回它覆盖范围内的东西。负载放在覆盖范围之外，测出来的既是零成本，也是假通过。」
- §2 对照脚本为什么存在：v2 照搬 gsd 那套的档位表与两阶段结构，**v2 不是 bench.py 的升级版**；
  `native_snapshot_bench.py` 的两个模式（pause 一次一份 / snapshot 多代并存）；分段映射表（原生无分段）
- §3 负载必须放在快照看得见的地方：CRIU 只认进程地址空间；文件那半的真实差异
- §4 恢复语义不同：原地回滚 / 另起新沙箱 / connect = resume；upload、产物是否落盘、touch 列；哪些列可并排、哪些只能并列不作差
- §5 一个真实发现：ARM 线原生 pause 的脏页判据把读也算脏 → 增量退化成工作集（`fbee6fcd1` 注释掉 `UFFDIO_COPY_MODE_WP`）；
  三套判据表；为什么对我们无影响。引 7、20
- §6 定性结论：「不是同一件事的两种实现，快的地方不一样，原因是快照范围不同」——gsd 拍的东西少，我们恢复不用重建进程；
  两条语义差别（回滚范围、失败面）。数字不在本篇
- §7 三张表并排之前的口径检查表（8 条）

**28 · 实测结果与达标判定**
- 预备：24 §4、26 §1
- §1 条件标签：两台机器差异表（机型 / 内核 / HDBSS / 根盘 fs / 介质 / 模板 / 二进制 sha）；为什么不能相减
- §2 功能正确性结果：950 08-29 verify 59/59；920B 09-04 全套（verify 59/59 三种组合、correctness 31/31、timing ALL CORRECT、
  loop 200/0、pause ×2、compat 13/2/0）；920B 08-24/25 两套。标注每轮的 SDK 与 `mem_mode` 判据是否有效
- §3 性能 · 950（HDBSS，真盘 ext4）：verify 顺带的墙钟（标明非基准）；`checkpoint_bench.py`【待测】；`bench-ckpt.py`【待测】；
  v2 / native【按已有数据填，缺的待测】。**表格骨架先立好，格子写「待测」**
- §4 性能 · 920B：**当前代码（2026-09-21，`deltabox-dev@4af2872c6`）的按改动量分档基准** ——
  口径与样本数、混合档主表、纯内存 / 纯文件 / 只读附表、拟合与反推（两条线同时在约 200 MB 处碰线）、
  分段拆解、「真实一步」场景、20 层链深、条件与不覆盖。旧栈那几轮移到 28A
- §5 **对照客户指标的判定表**：档位 × checkpoint（p50 / p99）× restore（浅 / 深）× 达标？——两台机器各一张；
  全量单列不参与判定并说明为什么；950 restore 0.100 s 恰在线上的说明；HDBSS 预期收益方向。**不达标的格子照写**
- §6 附表：与 gsd/CRIU 及原生 snapshot 的实测并列（920B 09-08；条件标签齐全；只并列不作差）
- §7 已知劣化与含义：连打 ×2.8（`manifest.json`）；cp1 排在全量回写后（fsync）；对使用场景的含义与缓解方向
- §8 尚未覆盖（合并原 17 §4.4 与 30 §5.1/5.2）

**28A · 历史实测数据（2026-08 / 09，旧栈）**（2026-09-21 拆出）
28 篇加入当前代码的分档基准之后超过单篇一万字的上限，于是把旧栈那几轮**原样**搬到本篇，一个数字不改。
- §0 先说清每一节的数据日期、代码版本、以及**哪几张表已被 28 篇 §4 / §5.1 取代**
- **新一轮没有覆盖、因此仍以本篇为准的**：XFS 方案全部数字、33 步深集 restore、60 秒持续连打、
  时机成本、链深 5 / 20 / 50 的真盘 XFS 数据
- 条件标签与每一轮的身份证仍然只在 28 篇 §1 维护，本篇引用过去
...
| # | 文件 | 标题 | 状态 |
|---|---|---|---|
| 30 | `30-extending.md` | 在这套方案上继续开发 | ● |
| 31 | `31-glossary-and-code-map.md` | 术语表与代码地图 | ● |

**30 · 在这套方案上继续开发**
- 代码地图：单仓库 `KASandbox_0904`、三个组件的职责边界、交付形态（`deltabox` 分支 + 手册）
- 改动指引，按任务组织：
...
> **第五部分并入后的一处全书性调整**：实测数字原先散在 00 §11、17 §4、01、03、19 等处，
> 现已统一收口到第 28 篇（缺口收口到 28 §8）。00 §11 压成一段结论 + 口径 + 链接，
> 17 §4 改为指路，其余各篇的引用改指 28。**新增数字一律只改 28。**
...
6. **18 → 19 → 20**
7. **24 → 25 / 26 → 27 → 28 → 29** —— 测试部分：方法篇先写，数据篇最后填
8. **30 → 31** —— 术语表与代码索引最后写，此时全书用词已经稳定
