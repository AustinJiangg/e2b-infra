...
> **2026-09-10：第五部分 6 篇（21–26）完成，全书 29 篇。** 下表状态已更新。
...
方法与数据分开：21–24 讲方法，一次写定；25 只放数据与判定，随 950 实测逐步填满；26 是客户面的操作篇。
全书的实测数字**只在 25 一处维护**，其他篇引用不复制。

| # | 文件 | 标题 | 状态 |
|---|---|---|---|
| 21 | `21-test-overview.md` | 测试体系总览 | ● |
| 22 | `22-functional-tests.md` | 功能正确性测试 | ● |
| 23 | `23-performance-methodology.md` | 性能测试：指标与方法 | ● |
| 24 | `24-cross-implementation.md` | 性能测试：三套方案的横向对照 | ● |
| 25 | `25-results-and-compliance.md` | 实测结果与达标判定 | ● |
| 26 | `26-acceptance-runbook.md` | 上机验收操作 | ● |
...
| 材料 | 位置 | 喂给 |
|---|---|---|
| 交付态脚本 docstring 与代码 | `../../benchmark/checkpoint_verify.py` `checkpoint_bench.py`；`checkpoint_bench_v2.py` `native_snapshot_bench.py`（尚未提交，在主工作区 `e2b-infra/benchmark/`） | 22 23 24 |
| 开发态工具箱 | `../test-950/*.py` `*.sh`、`README.md`、`MANIFEST.md` | 21 22 23 26 |
| `../test-950/耗时基准结论.md` | 两套对照、连打劣化、宿主分段 | 23 25 |
| `../test-950/reports/` | 950-verify-20260829、920b-kas0904-20260904、bench-ext4/xfs-20260824、ext4/xfs-2026082x、ramp、cow4k | 25 |
| `benchmark/checkpoint-bench-对比.md`（主工作区，未提交） | bench vs v2、与 gsd 的实测对比、已知缺陷 | 23 24 25 |
| `e2b-repo/三套快照方案的负载构造与测量口径.md`（工作区根，不在仓库） | 三套一张图、负载三种失真、负载放哪、恢复语义、ARM 原生脏页判据把读算脏 | 23 §2、24 全篇 |
...
- §2 三层：单元测试（只给位置与规模，引 27，不展开）/ 交付态验收 / 开发态工具箱。
...
- §1 **指标口径**：客户要求 → 本书的可测定义（按上面四条口径），一张「口径定义表」，25 篇的判定全按它
...
**24 · 性能测试：三套方案的横向对照**
- 预备：01、04、20、23；代码：`checkpoint_bench_v2.py`、`native_snapshot_bench.py`
...
- §8 尚未覆盖（合并原 17 §4.4 与 27 §5.1/5.2）
...
| # | 文件 | 标题 | 状态 |
|---|---|---|---|
| 27 | `27-extending.md` | 在这套方案上继续开发 | ● |
| 28 | `28-glossary-and-code-map.md` | 术语表与代码地图 | ● |
...
| `../test-950/` | **不动**。开发态工具箱，第 17、21、22、23、26、28 篇链接过去 |
| `../README.md` | ✅ 已重写，本系列为首要入口；随第五部分补入「要看测试证据 / 验收」的读法 |
| `../test-950/耗时基准结论.md` 等结论文档 | **不动**。第 23、25 篇吸收其内容，原文件仍是开发态记录 |
| `../../benchmark/checkpoint-bench-对比.md` | **不动**。第 23、24、25 篇吸收其内容 |
| 工作区根 `三套快照方案的负载构造与测量口径.md` | **不动**（不在仓库）。第 23 §2、24 篇吸收其内容 |

> **第五部分并入后的一处全书性调整**：实测数字原先散在 00 §11、17 §4、01、03、19 等处，
> 现已统一收口到第 25 篇（缺口收口到 25 §8）。00 §11 压成一段结论 + 口径 + 链接，
> 17 §4 改为指路，其余各篇的引用改指 25。**新增数字一律只改 25。**
...
6. **18 → 19 → 20**
7. **21 → 22 / 23 → 24 → 25 → 26** —— 测试部分：方法篇先写，数据篇最后填
8. **27 → 28** —— 术语表与代码索引最后写，此时全书用词已经稳定
