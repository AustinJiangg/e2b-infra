...
（[第 24 篇 §1](24-test-overview.md#1-为什么这套方案的测试要格外小心)把它列为第一种静默失效）。
...
1. **装 SDK 覆盖层是这条判据有效的前提。** 覆盖层的 `install.py` 在安装时就自检
   ...
   验收流程（[第 29 篇](29-acceptance-runbook.md)）把它列为前置步骤，就是为了这个。
...
与 950 上 2026-08-29 那份日志（工作区归档 `rollback-reports/950-verify-20260829/checkpoint-verify.log`）
...
| 段 | 看什么 |
|---|---|
| 开头两行 | `脏页后端 = hdbss / kvm-wp / off`、`产物落盘 ... 文件系统 = ext4 / xfs`。**这两行是后面所有数字的条件标签**，缺了就不能引用（[第 24 篇 §4](24-test-overview.md#4-测量纪律)） |
...
基准口径在 [第 26 篇 §1](26-performance-methodology.md#1-指标口径)。
...
> 完整结果表在 [第 28 篇 §2](28-results-and-compliance.md#2-功能正确性结果)。
...
> 920B 2026-09-04（ext4 方案，`kvm-wp`，产物落真盘 XFS，两个检查点交替）：200 次，
> failures = 0，p50 48 ms / p90 55 ms / max 402 ms，无一次超过 1 s
> （[第 28 篇 §2.1](28-results-and-compliance.md#21-总表)）。
