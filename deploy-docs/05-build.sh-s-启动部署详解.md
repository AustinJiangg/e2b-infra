...
- 自编译 `nbd.ko` 已手动加载（`ls /dev/nbd* | wc -l` = 512，runbook §0.2——脚本**不**自动加载）；
...
6. **NBD udev 规则**：`/etc/udev/rules.d/97-nbd-device.rules` 给 nbd 设备加 `nowatch`
   ...
   注意：**nbd 模块本身不在这里加载**（自编译 nbds_max=512 版，手动 insmod，runbook §0.2）。
...
- **firecracker**：优先用 `-i` 拷到 `/opt/e2b-infra/` 的
  `firecracker-v1.12.1-aarch64.tgz`（没有才尝试 wget），解压后装到
  `/fc-versions/v1.12.1/firecracker` 并加执行权限。

至此宿主机具备跑沙箱的全部静态条件：`/usr/bin` 双二进制、`/fc-envd`、`/fc-kernels`、
`/fc-versions`、大页、nbd（手动）、netns 工具链。
